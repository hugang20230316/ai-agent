...
v1 不修改 Codex 私有配置，不直接启用 hooks，也不读取历史会话做回填。
...
1. 手动候选阶段：用户要求沉淀时，由 agent 生成候选摘要，人工确认后写入 Obsidian。
2. 半自动节点阶段：在任务完成、compact 前、方案确认后，agent 主动给出候选摘要。
3. 受控写入阶段：确认 Obsidian Local REST API 和 MCP 可用后，启用候选写入。
4. 规则升级阶段：对 approved 条目做人工评审，少量迁移到 `rules/` 或 `skills/`。

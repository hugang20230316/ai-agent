...
本仓库只有 `rules/` 目录制定规则。`docs/` 目录只保存同步说明和迁移说明，不参与 Codex 规则加载。
...
只维护一份共用 `AGENTS.md`，Windows 和 Mac 都从它复制生成实际入口。同步仓库不再维护任何分平台公共入口文件。
...
说明文档可以解释为什么同步、怎么安装、哪些文件不能同步，但不能新增 Codex 行为规则。需要新增行为规则时，必须放入 `rules/*.md` 并由 `AGENTS.md` 引用。

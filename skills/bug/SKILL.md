...
3. `原因`: start with one tester/product-readable conclusion. If a defect is proven, state the first bad output, first bad transform, or exact code defect. If the issue is not reproduced, already recovered, or evidence is blocked, state that status and blocker first; unproven explanations must be marked as evidence-section hypotheses and must not replace the reason. Do not use "大概率/应该是/可能是" as the conclusion.

...
正文必须包含 `摘要`。`摘要` 只保留 `会话摘要` 和 `结论`。`会话摘要` 必须用独立的 `角色` / `语义标签` / `内容` Markdown 表格压缩保留关键对话结果，每条摘要独占一行；不得固定压成两轮，不得用左右两列把 `user` 和 `assistant` 挤在同一行，也不得把表格挂在列表项下导致 Obsidian 只显示管道文本。会话较复杂时，摘要应保留能还原任务目标、用户纠正或补充、关键决策、执行结果和验证边界的关键转折；没有新用户输入时，只在重大阶段保留 assistant 行，并说明它如何延续上一轮目标，不得伪造 user 行。`语义标签` 没有明确作用时留空；`纠正`、`抱怨`、`发火`、`辱骂` 等反馈类标签只有证据明确时才写，且必须同步到 frontmatter 的 `feedback_signal`、`feedback_target` 和可检索 `tags`。其余小节按 `log_kind` 选择，不强制所有日志套同一组字段：

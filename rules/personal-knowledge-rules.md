...
- 系统定时扫描器的语义是整理未处理会话，不是只整理机器刚好开机时最近时间窗内的会话。调度器可以每 3 小时触发一次，但扫描器必须能补处理错过运行窗口期间产生且尚未完成质量判断或写入的会话。
...
- 用户投诉默认是有意义反馈。用户对 agent 行为、规则失效、执行卡死、长任务中断、记录缺失、Obsidian 写入、扫描器调度、时间格式、自动化链路或验证结论的抱怨、纠正、质疑、发火或反复强调，默认进入候选判断并优先按 `feedback` 记录；只有证据明确显示没有对象、没有可复盘点、没有规则/执行/产品含义或存在安全风险时，才允许不记录，并必须保留不记录原因。
...
`created_at` 和非空 `reviewed_at` 必须使用北京时间 `YYYY-MM-DD HH:mm:ss`，例如 `2026-06-16 21:39:39`；不得使用 `2026-06-16T21:39:39+08:00` 这类 ISO 字符串，也不得只写日期。
...
正文必须包含 `记录理由`、`摘要` 和 `结论`。`记录理由` 用一句具体短句说明为什么这条值得保留，不能写“适合作为候选”“待人工复核”这类元描述。`摘要` 只保留独立的 `角色` / `语义标签` / `内容` Markdown 表格，不再写 `会话摘要：` 标题行；每条摘要独占一行，不得固定压成两轮，不得用左右两列把 `user` 和 `assistant` 挤在同一行，也不得把表格挂在列表项下导致 Obsidian 只显示管道文本。会话较复杂时，摘要应保留能还原任务目标、用户纠正或补充、关键决策、执行结果和验证边界的关键转折；没有新用户输入时，只在重大阶段保留 assistant 行，并说明它如何延续上一轮目标，不得伪造 user 行。`结论` 使用独立 `## 结论` 标题，写 1-2 句最终事实和验证边界，不写成 `结论：` 内联字段。`语义标签` 没有明确作用时留空；`纠正`、`抱怨`、`发火`、`辱骂` 等反馈类标签只有证据明确时才写，且必须同步到 frontmatter 的 `feedback_signal`、`feedback_target` 和可检索 `tags`。其余小节按 `log_kind` 选择，不强制所有日志套同一组字段：
...
不要把所有日志都写成 `关键事实`、`证据与资料`、`具体问题`、`解决方案`、`修改`、`验证情况`、`关联判断`。不要把原始聊天当附件或引用块粘进去；可以压缩，但关键语义、证据、资料和验证边界不能丢。中文正文写入前按 `humanizer-zh` 改写，去掉候选价值套话、协作客套、宣传腔、重复标题和为了凑格式写出的 `修改` / `验证` 小节。

...
- 共用模板：`AGENTS.md`
- Mac 差异：`AGENTS-mac.md`

只维护一份共用 `AGENTS.md`，Windows 和 Mac 都从它复制生成实际入口。Mac 相对共用入口的不同点写进 `AGENTS-mac.md`，不要再维护两份完整入口文件。模板可以上传到 GitHub；不要把带有本机绝对路径、账号、token 或项目路径的实际入口文件上传。
...
- `config-windows.md`：只适用于 Windows 的路径、脚本和本机状态说明
- `config-mac.md`：Mac 需要单独创建或迁移的路径、脚本和本机状态说明
...
- 平台相关路径只写进 `config-windows.md` 或 `config-mac.md`
